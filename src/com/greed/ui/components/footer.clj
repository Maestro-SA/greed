...
(defn- footer-link [href label]
  [:a {:href href :class "text-sm text-zinc-500 hover:text-zinc-900 transition-colors"}
   label])
  ...
  [:footer {:class "border-t border-zinc-200 bg-white"}
   [:div {:class "container mx-auto px-6 py-12"}
    [:div {:class "grid grid-cols-1 sm:grid-cols-4 gap-8 sm:gap-10"}
     [:div
      [:a {:href "/" :class "inline-block"}
       ...
       [:p {:class "mt-3 text-sm text-zinc-500 max-w-xs leading-relaxed"}
    ...
    [:div {:class "mt-10 pt-6 border-t border-zinc-100 flex flex-col sm:flex-row items-center justify-between gap-2"}
     [:p {:class "text-xs text-zinc-500"} "© 2026 greed. All rights reserved."]
     [:p {:class "text-xs text-zinc-500"} "Made in South Africa"]]]])
   ...
   [:div {:class "container mx-auto px-6 py-6"}
    [:div {:class "flex flex-col items-center justify-between gap-4 sm:flex-row"}
     ...
     [:nav {:class "flex items-center gap-6"}
      ...
      (footer-link "/contact" "Contact")]
     [:p {:class "text-xs text-zinc-500"} "© 2026 · Made in South Africa"]]]])
