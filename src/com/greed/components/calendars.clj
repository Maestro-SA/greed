       ...
       [:div
        ...
        [:template {:x-for "(day, index) in DAYS", :key "index"}
         [:div {:class "px-2 py-2 w-[14.28%]"}
          [:div {:class "text-gray-600 text-sm text-center"} [:span {:x-text "day"}]]]]]
       [:div
        ...
        [:template {:x-for "(date, dateIndex) in no_of_days", :key "dateIndex"}
         [:div {:class "text-center border-r border-b px-4 pt-2 h-[120px] w-[14.28%]"}
          [:div {:class "inline-flex w-6 h-6 items-center justify-center cursor-pointer text-center leading-none rounded-full transition ease-in-out duration-100"} [:span {:x-text "date"}]]]]]]]]
    (comment "Modal")
    [:div
     ...
     [:div
      ...
      [:div
       {:class
        "shadow absolute right-0 top-0 w-10 h-10 rounded-full bg-white text-gray-500 hover:text-gray-800 inline-flex items-center justify-center cursor-pointer",
      ...
      [:div
       ...
       [:div
        ...
        [:label
         {:class
          "text-gray-800 block mb-1 font-bold text-sm tracking-wide"}
         "Event title"]
        [:input
         {:class
        ...
        [:div
         ...
         [:select
          {"@change" "event_theme = $event.target.value;",
           :x-model "event_theme",
           :class
           "block appearance-none w-full bg-gray-200 border-2 border-gray-200 hover:border-gray-500 px-4 py-2 pr-8 rounded-lg leading-tight focus:outline-none focus:bg-white focus:border-blue-500 text-gray-700"}
          [:template {:x-for "(theme, index) in themes"} [:option {:x-text "theme"}]]]
         [:div
          {:class
       ...
       [:div
        ...
        [:button
         {:type "button",
          :class
          "bg-white hover:bg-gray-100 text-gray-700 font-semibold py-2 px-4 border border-gray-300 rounded-lg shadow-sm mr-2",
          "@click" "openEventModal = !openEventModal"}
         "Cancel"]
        [:button
         {:type "button",
          :class
          "bg-gray-800 hover:bg-gray-700 text-white font-semibold py-2 px-4 border border-gray-700 rounded-lg shadow-sm",
          "@click" "addEvent()"}
         "Save Event"]]]]]
    (comment "/Modal")]])
